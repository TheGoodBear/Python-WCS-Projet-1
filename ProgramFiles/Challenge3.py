...
def StartChallenge():
    ...
    RemainingPlayers = list(Var.CharactersData)
    PlayerInRemaingPlayers = (
        Player for Player
        in RemainingPlayers
        if Player["Category"] == ["Player"]
    )
    CurrentPlayer = random.randint(0, len(RemainingPlayers) - 1)
    ...
    LineOffset += RC.Print(Var.MessagesData["Challenge1"]["Start2"] + "\n\n",          
        TextVP["Y"] + LineOffset, TextVP["X"],
        JustifyText = RC.Justify.Center, 
        MaxColumns = TextVP["Width"])[0]
...
def ShowNextPlayer():
    """
        Switch off current player if any
        Go to next player
        Switch on next player and show its answer
    """
